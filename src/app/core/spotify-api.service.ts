...
export class SpotifyApiService {
  ...
  getUserTopArtists() {
    return this.http.get(
      `${this.baseUrl}/me/top/artists?time_range=medium_term&limit=10`
    );
  }

  recommendationsBasedOnSeeds(genres: string[], artists: string[]) {
    return this.http.get(
      `${this.baseUrl}/recommendations?limit=50&` +
  ...
  addTracksToPlaylist(playlistId: string, tracks: string[]) {
    return this.http.post(
      `${this.baseUrl}/playlists/${playlistId}/tracks?uris=${tracks
  ...
  getUserSavedTracks(tracks: string[]) {
    return this.http.get(
      `${this.baseUrl}/me/tracks/contains?ids=${tracks.join(",")}`
    );
  }
